...
function formatDiagnostic(diagnostic: Diagnostic, originalError: string): string {
  ...
  lines.push(`  │`)
  lines.push(`  │  For a full system check, run: crossnode-gtm doctor`)
  lines.push(`  └──────────────────────────────────────────────`)
  lines.push('')
  return lines.join('\n')
}
  ...
  return async (...args: Parameters<T>) => {
    try {
      ...
      if (diagnostic) {
        ...
        console.error(`  Try: crossnode-gtm doctor — for a full system health check`)
        console.error(`  Or:  crossnode-gtm doctor --report — to generate a diagnostic report`)
        console.error('')
