...
```bash
crossnode-gtm start
crossnode-gtm start --non-interactive    # Use env vars, skip prompts (CI/automation)
```
...
```bash
crossnode-gtm setup                      # Check current keys
crossnode-gtm setup --wizard             # Interactive key-by-key setup
```
...
```bash
crossnode-gtm onboard --website https://acme.com
crossnode-gtm onboard --linkedin https://linkedin.com/in/jdoe --website https://acme.com
crossnode-gtm onboard --knowledge docs/pitch.md docs/icp.md
```
...
crossnode-gtm doctor --report            # Write diagnostic report to file
```
...
```bash
crossnode-gtm test-run --count 10
```

### `update`
Pull the latest Crossnode GTM release without breaking your config.

```bash
crossnode-gtm update
```
...
crossnode-gtm campaign:create --title "Q2 Outbound" --auto-copy --segment-id seg-01
crossnode-gtm campaign:create --title "Q2 Outbound" --timezone "America/New_York" --send-window "09:00-17:00" --active-days "1,2,3,4,5"
crossnode-gtm campaign:create --title "Q2 Outbound" --start-at 2026-05-01 --delay-mode business
```
...
```bash
crossnode-gtm campaign:track
crossnode-gtm campaign:track --campaign-id abc123
...
```bash
crossnode-gtm campaign:schedule --campaign-id abc123 --send-window "10:00-16:00"
crossnode-gtm campaign:schedule --campaign-id abc123 --start-at none    # Clear scheduled start
```
...
```bash
crossnode-gtm campaign:monthly-report
```
...
```bash
crossnode-gtm campaign:create-sequence --sequence ./sequence.yaml --leads ./leads.csv
crossnode-gtm campaign:create-sequence --sequence ./sequence.yaml --linkedin-account acc-123 --dry-run
```
...
```bash
crossnode-gtm campaign:dashboard
crossnode-gtm campaign:dashboard --port 4000
```
...
```bash
crossnode-gtm leads:qualify --source csv --input ./leads.csv --dry-run
crossnode-gtm leads:qualify --source csv --input ./leads.csv
```
...
```bash
crossnode-gtm leads:scrape-post --url "https://linkedin.com/feed/update/urn:li:activity:123456"
```
...
```bash
crossnode-gtm leads:import --source csv --input data/leads/new-leads.csv
```
...
```bash
crossnode-gtm leads:find-linkedin --input ./leads.csv --output ./leads-with-linkedin.csv
crossnode-gtm leads:find-linkedin --input ./leads.csv --dry-run
```
...
```bash
crossnode-gtm leads:dedup --result-set rs-abc123
crossnode-gtm leads:dedup --result-set rs-abc123 --strategy fuzzy --slack-confirm
```
...
```bash
crossnode-gtm leads:export --result-set rs-abc123 --destination csv --output ./out.csv
crossnode-gtm leads:export --result-set rs-abc123 --destination lemlist
crossnode-gtm leads:export --result-set rs-abc123 --destination webhook --url https://hooks.example.com/leads
```
...
```bash
crossnode-gtm leads:suppress --file ./do-not-contact.csv
```
...
```bash
crossnode-gtm linkedin:answer-comments --url "https://linkedin.com/feed/update/urn:li:activity:123456" --dry-run
```
...
```bash
crossnode-gtm linkedin:reply-to-comments --url "https://linkedin.com/feed/update/urn:li:activity:123456" --template "Thanks {{name}}!"
crossnode-gtm linkedin:reply-to-comments --url <url> --templates "Reply A" "Reply B" --include-keywords pricing demo --max 50
```
...
```bash
crossnode-gtm email:create-sequence
```
...
crossnode-gtm email:send --to lead@example.com --subject "Quick question" --body "Hi there"
...
crossnode-gtm email:send --campaign-name "Q2 Outbound" --source ./leads.csv --sequence ./sequence.yaml
```
...
crossnode-gtm provider:test instantly
```
...
```bash
crossnode-gtm provider:remove brevo
```
...
```bash
crossnode-gtm keys:connect crustdata
crossnode-gtm keys:connect unipile --open
crossnode-gtm keys:connect             # Agnostic mode (pick provider in the UI)
```
...
```bash
crossnode-gtm connect-provider crustdata
```
...
```bash
crossnode-gtm notion:bootstrap
```
...
```bash
crossnode-gtm orchestrate "find 10 SaaS companies in Berlin with 50-200 employees"
crossnode-gtm orchestrate "research our top 3 competitors and compare their positioning"
crossnode-gtm orchestrate "find VP Engineering at companies using React, qualify them, and create a campaign"
```
...
```bash
crossnode-gtm agent:run --agent daily-linkedin-scraper --post-url "https://linkedin.com/..."
crossnode-gtm agent:run --agent my-custom-agent
```
...
```bash
crossnode-gtm agent:install --agent my-custom-agent
```
...
```bash
crossnode-gtm skills:search "email"
```
...
```bash
crossnode-gtm skills:install --github user/repo
crossnode-gtm skills:install --local ./my-skill
```
...
```bash
crossnode-gtm skills:info qualify-leads
```
...
```bash
crossnode-gtm skills:run qualify-leads --input company=acme.com --input role="VP Eng"
crossnode-gtm skills:run qualify-leads --input-file inputs.json --output result.json
```
...
```bash
crossnode-gtm skills:create
crossnode-gtm skills:create --format typescript
crossnode-gtm skills:create --non-interactive   # Fail instead of prompting
```
...
```bash
crossnode-gtm skills:validate ./my-skill/SKILL.md
```
...
```bash
crossnode-gtm tenant:onboard --tenant acme
crossnode-gtm tenant:onboard --tenant acme --adapter markdown-folder
crossnode-gtm tenant:onboard --tenant acme --no-scrape
```
...
```bash
crossnode-gtm framework:derive --tenant acme
```
...
```bash
crossnode-gtm memory:retrieve --query "what are our main competitors" --tenant acme
crossnode-gtm memory:retrieve --query "ICP pain points" --top-k 5
```
...
```bash
crossnode-gtm memory:dream --tenant acme
crossnode-gtm memory:dream --incremental
```
...
```bash
crossnode-gtm memory:index --tenant acme
```
...
```bash
crossnode-gtm context:sync --tenant acme
```
...
```bash
crossnode-gtm context:watch --tenant acme
```
...
```bash
crossnode-gtm results:review --result-set rs-abc123
```
...
```bash
crossnode-gtm research --target-type company --target acme.com --question "What's their pricing model?"
crossnode-gtm research --target-type person --target "Jane Doe @ Acme"
crossnode-gtm research --target-type topic --question "How are SaaS firms pricing AI features in 2026?" --max-sources 8
```
...
```bash
crossnode-gtm competitive-intel --url https://competitor.com
crossnode-gtm competitive-intel --url https://competitor.com --enrich
```
...
```bash
crossnode-gtm personalize --first-name Jane --last-name Doe --company acme.com --template ./template.md
```
...
```bash
crossnode-gtm signals:fetch --domain acme.com
crossnode-gtm signals:fetch --domain acme.com --types jobs,funding --no-cache
crossnode-gtm signals:fetch --domain acme.com --ttl-days 14
```
...
```bash
crossnode-gtm signals:enrich --result-set rs-abc123
crossnode-gtm signals:enrich --result-set rs-abc123 --types jobs,news --ttl-days 7
```
...
```bash
crossnode-gtm signals:show --domain acme.com
crossnode-gtm signals:show --domain acme.com --type jobs --limit 50
```
...
```bash
crossnode-gtm signals:similar --domain acme.com --limit 25
```
...
```bash
crossnode-gtm signals:watch --domains acme.com,beta.io
crossnode-gtm signals:watch --domains acme.com --types job-change,funding --force
```
...
```bash
crossnode-gtm signals:detect
crossnode-gtm signals:detect --type funding --company acme.com
```
...
```bash
crossnode-gtm signals:triggers list
crossnode-gtm signals:triggers set --signal funding --action slack --channel "#gtm-alerts" --template "{{company}} just raised"
crossnode-gtm signals:triggers set --signal hiring-surge --action campaign --campaign-id camp-123
```
...
```bash
crossnode-gtm framework:install outbound-saas-us
crossnode-gtm framework:install outbound-saas-us --destination notion --notion-parent <pageId> --auto-confirm
```
...
```bash
crossnode-gtm framework:run outbound-saas-us
crossnode-gtm framework:run outbound-saas-us --seed --open
```
...
```bash
crossnode-gtm framework:status outbound-saas-us
```
...
```bash
crossnode-gtm framework:logs outbound-saas-us
```
...
```bash
crossnode-gtm framework:resume outbound-saas-us --from-gate run-abc123
```
...
```bash
crossnode-gtm framework:disable outbound-saas-us
```
...
```bash
crossnode-gtm framework:remove outbound-saas-us
```
...
```bash
crossnode-gtm crm:setup
crossnode-gtm crm:setup --non-interactive   # Auto-accept all suggested mappings
```
...
```bash
crossnode-gtm crm:import
crossnode-gtm crm:import --dry-run
```
...
```bash
crossnode-gtm crm:push --result-set rs-abc123
crossnode-gtm crm:push --result-set rs-abc123 --dry-run
```
...
```bash
crossnode-gtm crm:sync
crossnode-gtm crm:sync --direction push
crossnode-gtm crm:sync --direction pull --dry-run
```
...
```bash
crossnode-gtm pipeline:create --name nightly-icp-refresh
crossnode-gtm pipeline:create --name nightly-icp-refresh --output ./pipelines/icp.yaml
```
...
```bash
crossnode-gtm pipeline:run --pipeline nightly-icp-refresh
crossnode-gtm pipeline:run --pipeline nightly-icp-refresh --dry-run
```
...
```bash
crossnode-gtm pipeline:resume --run-id run-abc123
```
...
```bash
crossnode-gtm pipeline:status --run-id run-abc123
```
...
```bash
crossnode-gtm visualize campaign-overview --data './data/campaigns/*.json'
crossnode-gtm visualize qualification-summary --data ./out/qualified.json --data ./out/unqualified.json
```
